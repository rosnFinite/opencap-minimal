...
from utils.utils import importMetadata, loadCameraParameters, getVideoExtension
from utils.utils import getDataDirectory, getOpenPoseDirectory, getMMposeDirectory
from utils.utilsChecker import saveCameraParameters
from utils.utilsChecker import calcExtrinsicsFromVideo
from utils.utilsChecker import isCheckerboardUpsideDown
from utils.utilsChecker import autoSelectExtrinsicSolution
from utils.utilsChecker import synchronizeVideos
from utils.utilsChecker import triangulateMultiviewVideo
from utils.utilsChecker import writeTRCfrom3DKeypoints
from utils.utilsChecker import popNeutralPoseImages
from utils.utilsChecker import rotateIntrinsics
from utils.utilsDetector import runPoseDetector, arrangeMMposePkl
from utils.utilsAugmenter import augmentTRC
from utils.utilsOpenSim import runScaleTool, getScaleTimeRange, runIKTool, generateVisualizerJson
...
def main(sessionName, trialName, trial_id, cameras_to_use=['all'],
         ...
         overwriteCamerasToUse=False, mmposeModelCfg=None, mmposeModelCkp=None):
    # %% High-level settings.
    # Camera calibration.
    ...
    generateVideo = True
    # This is a hack to handle a mismatch between the use of mmpose_utils and hrnet,
    # and between the use of OpenPose and openpose.
    if poseDetector == 'hrnet':
        poseDetector = 'mmpose_utils'
    elif poseDetector == 'openpose':
        poseDetector = 'OpenPose'
    if poseDetector == 'mmpose_utils':
    ...
    if runPoseDetection:
        ...
        for camName in cameraDirectories:
            ...
            if len(glob.glob(pathVideoWithoutExtension + '*')) == 0:
                ...
                if os.path.exists(
                        os.path.join(pathVideoWithoutExtension + getVideoExtension(pathVideoWithoutExtension))):
        ...
        if camerasToUse[0] == 'all':
            ...
            if not all([cam in cameras_available for cam in camerasToUse]):
                raise Exception(
                    'Not all specified cameras in camerasToUse have videos; verify the camera names or consider setting camerasToUse to ["all_available"]')
            else:
                camerasToUse_c = camerasToUse
        ...
        if poseDetector == "mmpose_utils":
            # check if modelCfg and modelCkp are provided for inference
            if mmposeModelCfg is None and mmposeModelCkp is None:
                raise ValueError("mmposeModelCfg and mmposeModelCkp parameter are None. Both need to be provided to "
                                 "run inference")

            if camerasToUse_c[0] == "all":
                cams = list(cameraDirectories.keys())
            else:
                cams = camerasToUse_c

            # für beide Cams
            pathVideoWithoutExtension = os.path.join(cameraDirectories[cams[0]],
                                                     trialRelativePath)

            videoExtension = getVideoExtension(pathVideoWithoutExtension)
            trialRelativePath += videoExtension  # fileName
            ...
            for camName in cameraDirectories:
                ...
                cmd_fr = ""
                CMD = "ffmpeg -loglevel error -y -i {}{} -q 0 {}".format(videoFullPath, cmd_fr, pathVideoRot)
                ...
                pathOutputVideo = os.path.join(cameraDirectories[camName], "OutputMedia_mmpose_" + str(bbox_thr),
                                               trialName)

                mmposeBoxDir = os.path.join("OutputBox_mmpose", trialName)
                pathOutputBox = os.path.join(cameraDirectories[camName], mmposeBoxDir)

                mmposePklDir = os.path.join("OutputPkl_mmpose_" + str(bbox_thr), trialName)
                pathOutputPkl = os.path.join(cameraDirectories[camName], mmposePklDir)

                os.makedirs(pathOutputVideo, exist_ok=True)
                os.makedirs(pathOutputBox, exist_ok=True)
                os.makedirs(pathOutputPkl, exist_ok=True)

                print(f"Successfully created folder structure inside {cameraDirectories[camName]}")
                ...
                # TODO: add to main parameters
                pathInferenceScript = os.path.join(os.getcwd(), "mmpose", "run_inference.py")
                mmpose_cmd = f"conda run -n mmpose python {pathInferenceScript} --video {pathVideoRot} --poseEstimation {pklPath} --boundingBox {bboxPath} --videoWithKeypoints {videoOutPath} --modelCfg {mmposeModelCfg} --modelCkp {mmposeModelCkp} --bboxThr {bbox_thr}"

                try:
                    print("Switching environment and running mmpose model...")
                    subprocess.run(mmpose_cmd, shell=True, check=True)
                except subprocess.CalledProcessError as e:
                    print(f"Exception during processing of mmpose inference script: {e}")

                print("\nPost-process data to have OpenPose-like file structure...")
                arrangeMMposePkl(poseInferencePklPath=pklPath, outputPklPath=ppPklPath)
    ...
    if runTriangulation:
        ...
        try:
            ...
            postAugmentationPklName = "keypoints_3d_openpose.pkl"
            if poseDetector == "mmpose_utils":
                postAugmentationPklName = "keypoints_3d_mmpose.pkl"
            with open(os.path.join(postAugmentationDir, postAugmentationPklName), "wb") as kp_file:
                pickle.dump(keypoints3D, kp_file)
        except Exception as e:
            if len(e.args) == 2:  # specific exception
                raise Exception(e.args[0], e.args[1])
            elif len(e.args) == 1:  # generic exception
    ...
    if runMarkerAugmentation:
        ...
        try:
            ...
            if len(e.args) == 2:  # specific exception
                ...
                raise Exception(exception, traceback.format_exc())
        if offset:
            # If offset, no need to offset again for the webapp visualizations.
    ...
    if runOpenSimPipeline:
            ...
            try:
                ...
                while thresholdPosition <= maxThreshold and not success:
                    try:
                        ...
                        logging.info(
                            f"Attempt identifying scaling time range with thresholdPosition {thresholdPosition} failed: {e}")
        ...
        # Write body transforms to json for visualizations.
...
if __name__ == "__main__":
    ...
    main(sessionName='',
         trialName="",
         trial_id='',
         poseDetector="mmpose_utils", genericFolderNames=True,
         dataDir='',
         mmposeModelCfg="",
         mmposeModelCkp="")
